...
def train(train_loader):
    ...
    param_configs = [dict(params=other_params, lr=lr, weight_decay=wd/lr)]
...
if __name__ == '__main__':
    ...
    os.makedirs('nets12_default', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_all)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets12_default/%s.pt' % uuid.uuid4())

    os.makedirs('nets12_minus_n40', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_minus40)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets12_minus_n40/%s.pt' % uuid.uuid4())
