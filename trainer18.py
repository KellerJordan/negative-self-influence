...
def train(train_loader):
    ...
    optimizer2 = torch.optim.SGD(other_params, lr=0.001, weight_decay=3.0/4, momentum=hyp['opt']['momentum'], nesterov=True)
    def get_lr(step):
...
if __name__ == '__main__':
    ...
    os.makedirs('nets18_default', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_all)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets18_default/%s.pt' % uuid.uuid4())

    os.makedirs('nets18_minus_n40', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_minus40)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets18_minus_n40/%s.pt' % uuid.uuid4())
