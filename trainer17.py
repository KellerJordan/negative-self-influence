...
def train(train_loader):
    ...
    other_params = [p for n, p in model.named_parameters() if 'norm' not in n and len(p.shape) < 4 and p.requires_grad]
    optimizer1 = RenormSGD(filter_params, lr=0.025, momentum=hyp['opt']['momentum'], nesterov=True)
    optimizer2 = torch.optim.SGD(other_params, lr=0.00064, weight_decay=3.0/2, momentum=hyp['opt']['momentum'], nesterov=True)
    def get_lr(step):
    ...
    #print(airbench.infer(model, airbench.CifarLoader('/tmp/cifar10', train=False), tta_level=0).abs().mean())
    #print(airbench.evaluate(model, airbench.CifarLoader('/tmp/cifar10', train=False), tta_level=2))
    #1/0
    return model
...
if __name__ == '__main__':
    ...
    os.makedirs('nets17_default', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_all)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets17_default/%s.pt' % uuid.uuid4())

    os.makedirs('nets17_minus_n40', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_minus40)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets17_minus_n40/%s.pt' % uuid.uuid4())
