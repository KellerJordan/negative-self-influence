...
if __name__ == '__main__':
    ...
    os.makedirs('nets14_default', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_all)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets14_default/%s.pt' % uuid.uuid4())

    os.makedirs('nets14_minus_n40', exist_ok=True)
    for _ in tqdm(range(1000)):
        net = train(loader_minus40)
        outs = airbench.infer(net, loader_all)
        torch.save(outs, 'nets14_minus_n40/%s.pt' % uuid.uuid4())
